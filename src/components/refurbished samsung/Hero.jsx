...
const benefits = [
  {
    ...
    text: "Every Samsung device is inspected, cleaned and function-tested before sale.",
    icon: FaSyncAlt,
  },
  {
    title: "SamsungCare",
    text: "Support options are available for extra reassurance after purchase.",
    icon: FaMobileAlt,
  },
  {
    title: "Protected purchase",
    text: "Warranty cover, clear condition notes and careful packaging are included.",
    icon: FaShieldAlt,
  },
  {
    title: "Fast delivery",
    text: "In-stock refurbished Samsung phones are prepared quickly for dispatch.",
    icon: FaBoxOpen,
  },
];
...
const getCartItem = (product) => ({
  id: `samsung-${product.name.toLowerCase().replace(/\s+/g, "-")}`,
  name: product.name,
  image: product.image,
...
const Hero = () => {
  ...
  return (
    <div className="bg-white text-[#121722]">
      <style>
        {`
          @keyframes rmgFadeUpSamsung {
            from { opacity: 0; transform: translateY(18px); }
            to { opacity: 1; transform: translateY(0); }
          }

          .rmg-samsung-fade-up {
            animation: rmgFadeUpSamsung 0.65s ease both;
          }
        `}
      </style>
      ...
      <section className="mx-auto grid min-h-[520px] max-w-[1720px] items-center gap-10 px-6 py-14 sm:px-10 lg:grid-cols-[1fr_0.9fr] lg:px-20 lg:py-16">
        <div className="rmg-samsung-fade-up">
          ...
          <p className="mt-5 max-w-[800px] text-[16px] leading-relaxed text-[#333844] sm:text-[18px]">
            Shop carefully tested refurbished Samsung phones with clear pricing,
            warranty support and reliable condition checks.
          </p>
        ...
        <div className="rmg-samsung-fade-up flex justify-center lg:justify-end [animation-delay:0.12s]">
          <img
            src="/imgi_11_7d3ea32f601b45ab833bdde8e082d746_samsungs.png"
            alt="Refurbished Samsung phones"
      ...
      <section
        id="refurbished-samsung"
        className="mx-auto max-w-[1720px] px-6 py-14 sm:px-10 lg:px-20 lg:py-16"
      >
        <div className="rmg-samsung-fade-up flex flex-wrap items-end justify-between gap-4">
          <div>
        ...
        <div className="rmg-refurb-grid mt-10 grid gap-7 sm:grid-cols-2 xl:grid-cols-3">
          {products.map((product, index) => (
            <div
              key={product.name}
              className="rmg-refurb-card group rmg-samsung-fade-up block h-[430px] [perspective:1200px]"
        ...
        <div className="mx-auto grid max-w-[1600px] gap-8 md:grid-cols-2 xl:grid-cols-4">
          {benefits.map((benefit, index) => {
            ...
            return (
              <div
                key={benefit.title}
                className="rmg-samsung-fade-up text-center"
      ...
      {selectedProduct && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="samsung-refurbished-popup-title"
        >
          <button
            type="button"
            ...
            <div className="grid gap-7 p-5 sm:p-7 lg:grid-cols-[0.95fr_1.05fr]">
              ...
              <div>
                <p className="text-[16px] leading-relaxed text-[#3e4650] sm:text-[17px]">
                  {selectedProduct.detail} Each refurbished Samsung is cleaned,
                  tested and prepared with clear condition checks before sale.
                </p>
                <div className="mt-6 grid gap-3 text-[15px] font-semibold text-[#252936]">
                  <div className="rounded-md bg-[#f6f7f8] px-4 py-3">
                    Display, charging, camera and speaker checked
                  </div>
                  <div className="rounded-md bg-[#f6f7f8] px-4 py-3">
                    Fold and S Pen checks included where applicable
                  </div>
                  <div className="rounded-md bg-[#f6f7f8] px-4 py-3">
